...
Astriarch.AI = {
	...
	ComputerTakeTurn: function(/*Player*/ player){
		...
		Astriarch.AI.computerSetPlanetBuildGoals(player, ownedPlanetsSorted);


		Astriarch.AI.computerBuildImprovementsAndShips(player, ownedPlanetsSorted);
		...
		Astriarch.AI.computerSendShips(player, ownedPlanetsSorted);
	...
	computerSetPlanetBuildGoals: function(/*Player*/ player, ownedPlanetsSorted){   
		...
		for (var i in planetCandidatesForNeedingShips)
		{
			...
			else if (Astriarch.GameModel.Turn.Number % 4 == 0 && buildDefenders)
			{
	...
	computerBuildImprovementsAndShips: function(/*Player*/ player, ownedPlanetsSorted){
		...
		for (var i in ownedPlanetsSorted)
		{
			...
			if (p.BuildQueue.length == 0)
			{
				if (p.Id in player.PlanetBuildGoals)
				{
					...
					{
						p.EnqueueProductionItemAndSpendResources(ppi, player);
	...
	computerSendShips: function(/*Player*/ player, ownedPlanetsSorted)
	{
			...
			if (p.PlanetaryFleet.GetPlanetaryFleetMobileStarshipCount() > 0)
			{
				...
				else
				{
					...
					if (player.Type == Astriarch.Player.PlayerType.Computer_Hard || player.Type == Astriarch.Player.PlayerType.Computer_Expert)
					{
						...
						var closestUnownedPlanet = Astriarch.GameTools.GetClosestUnownedPlanet(player, p, minDistanceObject);//Planet
						if (closestUnownedPlanet != null)
						{
							...
							else if (player.KnownPlanets[closestUnownedPlanet.Id])
							{
			...
			for (var i in Astriarch.GameModel.Planets)
			{
				...
				if (p.Owner != player && !player.PlanetContainsFriendlyInboundFleet(p))//exploring/attacking inbound fleets to unowned planets should be excluded
				{
					if (!player.KnownPlanets[p.Id] ||
						...
						(Astriarch.GameModel.Turn.Number - player.LastKnownPlanetFleetStrength[p.Id].TurnLastExplored) > 20))//TODO: figure out best value here (could be shorter if planets are closer together)
					{
		...
		if (player.HomePlanet != null)//just to make sure
		{
			...
			{
				var planetDistanceComparer = new Astriarch.Planet.PlanetDistanceComparer(player.HomePlanet)
			...
			else
			{
				...
				var planetValueDistanceStrengthComparer = new Astriarch.Planet.PlanetValueDistanceStrengthComparer(player.HomePlanet, player.LastKnownPlanetFleetStrength)
		...
		for (var i = planetCandidatesForInboundAttackingFleets.length - 1; i >= 0; i--)
		{
			...
			if (player.Type == Astriarch.Player.PlayerType.Computer_Easy || player.Type == Astriarch.Player.PlayerType.Computer_Normal)
			{
				var planetDistanceComparer = new Astriarch.Planet.PlanetDistanceComparer(pEnemyInbound);
			...
			{
				var planetValueDistanceStrengthComparer = new Astriarch.Planet.PlanetValueDistanceStrengthComparer(pEnemyInbound, player.LastKnownPlanetFleetStrength)
			...
			if (!fleetSent && planetCandidatesForInboundReinforcements.length > 0)
			{
				...
				var distanceFromPlanetToReinforceToEnemy = Astriarch.GameModel.GameGrid.GetHexDistance(pEnemyInbound.BoundingHex, planetToReinforce.BoundingHex);
				...
				for (var r = planetCandidatesForSendingShips.length - 1; r >= 0; r--)
				{
					...
					if (Astriarch.GameModel.GameGrid.GetHexDistance(pEnemyInbound.BoundingHex, pFriendly.BoundingHex) < distanceFromPlanetToReinforceToEnemy)
						break;
