...
Astriarch.View.CanvasNotSupported = function() {
	...
	$('#canvasContainer').html('<p style="color:white">Astriarch HTML 5 version not supported by your browser.  You can play the silverlight plugin by clicking <a href="silverlight.html">here</a>. In 5 seconds you will be taken to the silverlight version automatically.</p>');
...
Astriarch.View.UpdateStartupOptionsBasedOnOpponentCount = function() {
	var numberOfOpponents = Number($('input:radio[name=OpponentCountRadioGroup]:checked').val());
	$('#Computer2Panel').hide();
	$('#Computer3Panel').hide();
	
	$('#StarfieldPanel1').show();
	$('#StarfieldPanel2').show();

	if (numberOfOpponents >= 2)
	{
		$('#Computer2Panel').show();
		$('#StarfieldPanel1').hide();
		
		$('#StarfieldRadio2').prop('checked', true);
	}
	if (numberOfOpponents >= 3)
	{
		$('#Computer3Panel').show();
		$('#StarfieldPanel2').hide();
		
		$('#StarfieldRadio3').prop('checked', true);;
	}
};
...
Astriarch.View.SetupGraphicalDOMElements = function() {
	...
	$("#NewCampaignGameButton").hide();
	$("#ResumeGameButton").button();
	$("#ResumeGameButton").button('disable');
	
	if(Astriarch.SavedGameInterface.savedGameExists())
	{
		$("#ResumeGameButton").button('enable');
	}
	...
	$("#ResumeGameButton").click(
		function() {
			$('#mainMenu').hide();
			if(Astriarch.SavedGameInterface.loadGame())
			{
				...
				//there was a problem loading the game, just let them start a new one
				alert('There was an error loading your saved game, you will have to start a new game instead.');
				Astriarch.View.ShowNewGameOptions();
			}
		}
	);
	...
	$('select#Computer1DifficultyComboBox').selectmenu({style:'popup',width:90});
	$('select#Computer2DifficultyComboBox').selectmenu({style:'popup',width:90});
	$('select#Computer3DifficultyComboBox').selectmenu({style:'popup',width:90});
	
	$('select#PlanetsPerSystemComboBox').selectmenu({style:'popup', width:55});
	
	$("#OpponentCountRadio1").change( function() { Astriarch.View.UpdateStartupOptionsBasedOnOpponentCount(); } );
	$("#OpponentCountRadio2").change( function() { Astriarch.View.UpdateStartupOptionsBasedOnOpponentCount(); } );
	$("#OpponentCountRadio3").change( function() { Astriarch.View.UpdateStartupOptionsBasedOnOpponentCount(); } );
	
	$("#StartGameOptionsOkButton").button();
	$("#StartGameOptionsOkButton").click(
		function() {
			Astriarch.GameController.SetupNewGame();
			$('#startGameOptionsContainer').hide();
...
Astriarch.View.ShowPlanetView = function() {
	var p = Astriarch.GameModel.GameGrid.SelectedHex.PlanetContainedInHex;//Planet
...
Astriarch.View.StartSendShips = function() {
	...
	Astriarch.View.sendShipsFromHex = Astriarch.GameModel.GameGrid.SelectedHex;
};
	...
	if (dialogResult == true)
	{
		...
		if (Astriarch.SendShipsControl.CreatedFleet != null)
		{
			...
			if (Astriarch.SendShipsControl.pSource.PlanetaryFleet.GetPlanetaryFleetMobileStarshipCount() == 0)
			{
				...
				dp.UpdatePlanetDrawingForPlayer(Astriarch.GameModel.MainPlayer);
			}
		}
	}
...
Astriarch.View.PlayfieldClicked = function(pos) {
	...
	var hClicked = Astriarch.GameModel.GameGrid.GetHexAt(point);//Hexagon

	if (hClicked != null && hClicked.PlanetContainedInHex != null)
	{
		Astriarch.GameModel.GameGrid.SelectHex(hClicked);
		...
		{
			if (Astriarch.View.sendShipsFromHex != Astriarch.GameModel.GameGrid.SelectedHex)
			{
				var distance = Astriarch.GameModel.GameGrid.GetHexDistance(Astriarch.View.sendShipsFromHex, Astriarch.GameModel.GameGrid.SelectedHex);

				//show select ships to send dialog
				var p1 = Astriarch.View.sendShipsFromHex.PlanetContainedInHex;
				var p2 = Astriarch.GameModel.GameGrid.SelectedHex.PlanetContainedInHex;
...
Astriarch.View.PlayfieldDoubleClicked = function(pos) {
	...
	if (hClicked != null && hClicked.PlanetContainedInHex != null)
	{
		if (hClicked.PlanetContainedInHex.Owner == Astriarch.GameModel.MainPlayer)
		{
...
Astriarch.View.selectPlanet = function(/*Planet*/ p) {
	Astriarch.GameModel.GameGrid.SelectHex(p.BoundingHex);
	Astriarch.View.updateSelectedItemPanelForPlanet();
}

Astriarch.View.updatePlayerStatusPanel = function() {
	var mainPlayer = Astriarch.GameModel.MainPlayer;
...
Astriarch.View.updateSelectedItemPanelForPlanet = function() {
	...
	if (Astriarch.GameModel.GameGrid.SelectedHex != null)
	{
		var p = Astriarch.GameModel.GameGrid.SelectedHex.PlanetContainedInHex;//Planet
		var mainPlayer = Astriarch.GameModel.MainPlayer;
		sb += "--- Planet " + p.Name + " ---<br />";
		...
		var planetKnownByPlayer = mainPlayer.PlanetKnownByPlayer(p);
		...
		else//the main player has explored it
		{
			...
			if (mainPlayer.LastKnownPlanetFleetStrength[p.Id])
			{
				lastKnownFleet = mainPlayer.LastKnownPlanetFleetStrength[p.Id];
				lastKnownOwner = lastKnownFleet.LastKnownOwner;
			}
				
			sb += Astriarch.GameTools.PlanetTypeToFriendlyName(p.Type) + "<br />";

			if (p.Owner == mainPlayer || Astriarch.GameModel.ShowUnexploredPlanetsAndEnemyPlayerStats)//for now only show details for owned planets
			{
			...
			{
				sb += Astriarch.GameTools.PlanetOwnerToFriendlyName(p, lastKnownOwner) + "<br />";

				var lastKnownPlanetFleet = lastKnownFleet.Fleet;//Fleet
				var turnSinceExplored = Astriarch.GameModel.Turn.Number - lastKnownFleet.TurnLastExplored;
...
Astriarch.View.updateCanvasForPlayer = function() {
	...
	{
		Astriarch.View.DrawnPlanets[i].UpdatePlanetDrawingForPlayer(Astriarch.GameModel.MainPlayer);//TODO: update for multiplayer
	}
	Astriarch.View.CanvasPlayfieldLayer.needsDisplay = true;
};
